/**
 * BlackHole.tsx — a real one. Light bent, ray by ray, no picture of a ring.
 *
 * The landing hero's right column is a bone plate floating in a large, empty
 * ink field. This fills that field with the thing the product is named after,
 * and it is not an illustration of a black hole — it is one. Every pixel fires
 * a ray out of the camera and walks it backwards through curved space until
 * the ray falls through the horizon, leaves for the stars, or cuts the gas
 * disc — often three or four times over, because a ray can loop the hole and
 * come back. The disc is drawn once. The halo above it, the halo below it and
 * the hard thin ring hugging the shadow are the same disc, seen again through
 * bent light. That is why they line up: nobody lined them up.
 *
 * Units are set by the horizon: r = 1 is the event horizon. The photon sphere
 * sits at 1.5, the shadow the camera sees is 2.6 across the radius, and the
 * disc starts at 3 — the innermost orbit gas can hold in this geometry. Those
 * numbers are not tuned. They fall out of the metric.
 *
 * ── How it lives on this page ─────────────────────────────────────────────
 *
 * · It is a backdrop, not a centrepiece. The hero's plate stays exactly where
 *   it was and stays legible; the hole is behind it, inked into the page by a
 *   radial mask so there is no canvas edge to see — the glow simply thins out
 *   into the ink the page is already painted on.
 *
 * · The camera is scroll-driven and nothing else. As the hero leaves, the
 *   camera swings around the hole and closes in a little. There are already
 *   three pointer-chasing effects on this hero (parallax, wash, magnet); a
 *   fourth would be noise, and on glass `pointermove` fires during the scroll
 *   drag anyway.
 *
 * · It costs what it costs, so it is careful about when it costs it. Nothing
 *   is drawn while the hero is off screen or the tab is hidden; the render
 *   scale and step count drop on a software rasteriser; the step count walks
 *   itself down if frames get long; and a reduced-motion reader gets sixteen
 *   settled passes of one still frame and no animation loop at all.
 *
 * · Narrow screens do not mount it. Below 981px the hero stacks to one column
 *   and there is no empty right half to fill — the phone face of this app is
 *   a different UI, and a WebGL ray marcher is not what that screen needs.
 *
 * Knobs are all props with defaults tuned for the hero slot. See
 * docs/BLACKHOLE.md for what each one does to the picture.
 */
...
void gasAt(vec3 p, float rd, float dt, out float dens, out vec3 tint, out float heat) {
  ...
  tint = mix(tint, uHot, smoothstep(0.68, 1.18, heat));
}
...
/**
 * Folds the new frame into the running average.
 *
 * The gas moves, and `uAlpha` is the price of that — the more weight the
 * history keeps, the cleaner the still and the more the moving gas smears
 * behind itself. When the camera itself moves (the hero is being scrolled)
 * the history stops being a valid estimate of the same sky, so the caller
 * raises the weight toward a hard reset and the ghosting disappears with it.
 */
const BLEND_FRAG = `
precision highp float;
varying vec2 vUv;
...
/**
 * Composite, and the last word on how the hole meets the page.
 *
 * There is deliberately no scrim layer here — no hard edge to veil off. The
 * element this draws into is masked in CSS, so the falloff is the page's, not
 * the shader's, and the picture can be any size without the fade changing
 * shape. What is left here is the tone curve, the corners, and a grain of
 * dither: without the dither these long, dark ramps band into rings.
 */
...
void main() {
  ...
  // Corners down. The page's own mask finishes the job at the rim, so this
  // only has to take the middle's contrast down a little as it goes out.
  vec2 d = vUv - 0.5;
...
/** Hex to linear light. The shader works in linear and gamma-encodes at the
 *  end. Memoised, because this runs three times per frame and the colours
 *  almost never change. */
const linearCache = new Map<string, [number, number, number]>();
function hexToLinearCached(hex: string): [number, number, number] {
  const hit = linearCache.get(hex);
  if (hit) return hit;
  const out = hexToLinear(hex);
  linearCache.set(hex, out);
  return out;
}
...
/** True while the query matches. Watchable, because layout may cross it later. */
...
export interface BlackHoleProps
  ...
  /** Camera distance from the hole, in horizon radii. Closer is bigger. */
  distance?: number;
  /** How far the camera sits above the disc plane, in degrees. Near 0 the disc
   *  is edge-on and the far side arches over the shadow as a halo. */
  elevation?: number;
  /** Where the camera sits around the hole, in degrees. */
  azimuth?: number;
  /** Turns the picture about the line of sight, so the disc runs diagonally. */
  roll?: number;
  /** Vertical field of view, in degrees. */
  fov?: number;
  /** Inner edge of the disc, in horizon radii. 3 is the innermost stable orbit. */
  diskInner?: number;
  /** Outer edge of the disc. */
  diskOuter?: number;
  /** Half-thickness of the disc at its inner edge. It flares outward. */
  ...
  brightness?: number;
  /** Turns of the inner rim per second. The rest of the disc lags, hard. */
  spinSpeed?: number;
  /** Size of the turbulence in the gas. Higher is finer. */
  grain?: number;
  /** Relativistic beaming, 0 to 1. 1 is the truth; the film everyone has seen
   *  set it to 0 to keep the ring even. */
  doppler?: number;
  ...
  cool?: string;
  /** Background stars, 0 to 2. They are lensed along with everything else. */
  stars?: number;
  /** Bloom, 0 to 2. */
  glow?: number;
  ...
  vignette?: number;
  /** Steps each ray may take. This is the one dial that costs real time. */
  steps?: number;
  /** Render scale, 0.4 to 1. Drop it before you drop `steps`. */
  resolution?: number;
  /** Cap on device pixel ratio. */
  maxDpr?: number;
  /** Where the hole sits on the canvas, top-left origin, as fractions. Match
   *  this to the element's CSS mask or the fade will not be centred on it. */
  focus?: [number, number];
  /** Degrees of azimuth the camera swings through as the page scrolls past. */
  swing?: number;
  /** Fraction of the start distance the camera closes in over the same scroll. */
  dolly?: number;
}
...
export function BlackHole({
  ...
  grain = 0.5,
  doppler = 0.4,
  // Bone at the rim, brass through the middle, deep vermilion outside. The
  // disc is hottest where it is fastest, and the site's palette happens to run
  // in exactly that order — so the picture is of something and in brand at the
  // same time, which is the only reason it is allowed on this page at all.
  hot = "#eef1f6",
  mid = "#c7a46a",
  cool = "#8a241d",
  stars = 0.35,
  ...
  // Below the hero's one-column breakpoint there is no right half to fill, and
  // this is the most expensive thing on the page. Never mount it there.
  const wide = useMedia("(min-width: 981px)");
  const ready = useReady();

  const hostRef = useRef<HTMLDivElement | null>(null);
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const playRef = useRef<((on: boolean) => void) | null>(null);

  const props = useRef({
    distance, elevation, azimuth, roll, fov, diskInner, diskOuter,
    diskThickness, diskDensity, brightness, spinSpeed, grain, doppler, hot,
    mid, cool, stars, glow, exposure, vignette, steps, resolution, maxDpr,
    focus, swing, dolly,
  });
  props.current = {
    distance, elevation, azimuth, roll, fov, diskInner, diskOuter,
  ...
  useEffect(() => {
    ...
    // happy-dom, a locked-down browser, a canvas someone else already claimed:
    // any of these can throw rather than return null. The hero must survive it.
    ...
    /**
     * Take the canvas out of the picture and leave the ink page behind it. A
     * canvas whose context has died does not go quietly: it paints white, or
     * the browser's broken-image mark, straight over the hero — which is worse
     * than showing nothing at all.
     */
    function giveUp(why: string) {
      host!.dataset.blackhole = why;
    ...
    function resize() {
      ...
      const scale = software
        ? 0.34
        : clamp(props.current.resolution * tier, 0.3, 1);
    ...
    let lastFrame = 0;
    let running = true;
    /** Off until the boot curtain lifts. One still frame is drawn at mount. */
    let playing = false;
    let visible = true;
    let raf = 0;

    // Quality is not a constant, it is a negotiation with the machine it is
    // running on. `tier` scales the render target; `steps` walks itself down
    // when frames stretch. Both only ever move in small, hysteretic steps, so
    // the picture cannot visibly pump.
    let tier = 1;
    let budget = software ? 120 : Math.max(110, Math.round(props.current.steps));
    let ema = 0;
    let sinceTune = 0;
    ...
    /**
     * How far the hero has been scrolled out of the viewport, 0 to 1.
     *
     * The host's parent is `.hero__void`, the box stretched across the whole
     * hero, so its rect already answers "how far has this section travelled" —
     * no scroll listener, no ScrollTrigger, and no second element to keep in
    ...
    // Last frame's camera, so the blend can tell a still shot from a moving one.
    let lastAz = NaN;
    let lastEl = NaN;
    let lastDist = NaN;
    ...
    function render(t: number) {
      ...
      // Camera: an orbit about the hole, at a fixed height above the disc. The
      // scroll is the whole of the interaction: as the hero leaves, the camera
      // swings around the hole and closes in, which reads as the void turning
      // to watch the page go rather than as a background sliding past.
      const p = REDUCED ? 0 : scrollProgress();
      const eased = p * p * (3 - 2 * p);
      const az = (C.azimuth + C.swing * eased) * RAD;
      const el = clamp(C.elevation + 2 * eased, -88, 88) * RAD;
      const dist = Math.max(2.2, C.distance * (1 - C.dolly * eased));
      ...
      const hot = hexToLinearCached(C.hot);
      const mid = hexToLinearCached(C.mid);
      const cool = hexToLinearCached(C.cool);
      ...
      // since there is no history worth keeping — and so does every frame of a
      // scroll, because then last frame's sky is not this frame's sky and the
      // average would smear the hole across itself instead of cleaning it.
      let motion = 0;
      if (!Number.isNaN(lastAz)) {
        motion =
          Math.abs(az - lastAz) +
          Math.abs(el - lastEl) +
          2 * Math.abs(dist - lastDist) / Math.max(1, dist);
      }
      lastAz = az;
      lastEl = el;
      lastDist = dist;
      const alpha = settled === 0 ? 1 : clamp(0.14 + 9 * motion, 0.14, 1);
      ...
      gl!.uniform1f(compProg.u.uVignette!, clamp(C.vignette, 0, 1));
      gl!.uniform1f(compProg.u.uSeed!, (t * 60) % 1000);
      draw();
    }

    /** One gaussian round, one axis. */
    ...
    function tick(now: number) {
      ...
      if (!visible || !playing) { lastFrame = now; return; }
      const dt = lastFrame ? Math.min(0.05, (now - lastFrame) / 1000) : 0;
      lastFrame = now;
      const spend = dt > 0 ? dt * 1000 : 0;
      if (!REDUCED) clock += dt;
      render(clock);

      // Walk the step count toward whatever this machine can actually afford.
      // Slow by a hair over a second and the budget comes down; fast for the
      // same and it comes back up. It never lowers the target scale, because
      // rebuilding five targets mid-scroll costs more than it saves.
      if (spend > 0) {
        ema = ema ? ema * 0.94 + spend * 0.06 : spend;
        if (++sinceTune >= 48) {
          sinceTune = 0;
          const cap = software ? 120 : Math.max(110, Math.round(props.current.steps));
          if (ema > 28 && budget > 110) budget = Math.max(110, budget - 40);
          else if (ema < 12 && budget < cap) budget = Math.min(cap, budget + 20);
          if (software && ema > 60 && tier > 0.4) {
            tier = 0.34;
            resize();
          }
        }
    ...
    // A resize while nothing is animating — the boot window, or a reader who
    // asked for no motion — needs the still re-drawn, because the new targets
    // are empty. It must not be re-drawn *per resize event*: a window drag
    // fires dozens, and sixteen synchronous renders each is a frozen browser.
    // So the settle waits for the resize to stop.
    let settleTimer: ReturnType<typeof setTimeout> | undefined;
    const ro = new ResizeObserver(() => {
      resize();
      if (!REDUCED && playing) return;
      clearTimeout(settleTimer);
      settleTimer = setTimeout(() => { settle(16); }, 180);
    });
    ro.observe(host);
    ...
    // Everything above is set up once, at mount, so the first frame is already
    // on screen when the boot curtain lifts. Playback is the only thing the
    // outside world gets to switch — and it stays off until the curtain is up,
    // because frames drawn behind it are frames nobody sees.
    playRef.current = (on: boolean) => {
      if (on === playing) return;
      playing = on;
      lastFrame = 0;
      // Pausing is not the same as freezing: hand the average the passes it
      // would have collected over the next second, so what is left on screen
      // is a settled picture rather than the last noisy one.
      if (!on && !REDUCED) settle(16);
    };
    ...
    return () => {
      ...
      // empty message, which is a miserable thing to debug. Deleting what was
      // allocated is enough; the context goes when the canvas does.
    };
  }, [wide]);

  useEffect(() => {
    playRef.current?.(ready && !REDUCED);
  }, [ready]);

  return (
    <div
      ref={hostRef}
      className={`blackhole ${className}`}
      {...rest}
    >
      {wide ? (
        <canvas ref={canvasRef} aria-hidden="true" className="blackhole__canvas" />
      ) : null}
    </div>
  );
}
